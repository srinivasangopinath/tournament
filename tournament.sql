...
1 as points,
1 as match
from players p inner join matches m on (p.player_id=m.winner)
union
select p.player_id,
p.name,
m.match_num,
...
0 as points,
1 as match
from players p inner join matches m on (p.player_id=m.loser)
union
select p.player_id,
p.name,
m.match_num,
...
select player_id,name,sum(points) wins, sum(match) matches from match_view 
